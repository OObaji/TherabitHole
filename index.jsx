...
// --- DATA SOURCE ---
// A curated list of ~150 diverse interests to simulate the "thousands" feel.
...
const Header = ({ viewMode, setViewMode, activeTab, setActiveTab, searchTerm, setSearchTerm, totalItems }) => (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row items-center justify-between py-4 space-y-4 md:space-y-0">
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">The Rabbit Hole</h1>
            <p className="text-xs text-slate-400 font-medium">Explore {totalItems} curiosities</p>
          </div>
        </div>
        ...
        <div className="relative w-full md:w-96 group">
          ...
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-slate-700 rounded-full leading-5 bg-slate-800 text-slate-100 placeholder-slate-500 focus:outline-none focus:bg-slate-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 sm:text-sm transition-all shadow-inner"
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto justify-end">
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800'}`}
            title="Grid View"
          >
            <Grid size={20} />
          </button>
          <button 
            ...
            className={`p-2 rounded-lg transition-all ${viewMode === 'stumble' ? 'bg-fuchsia-600 text-white shadow-lg shadow-fuchsia-500/20' : 'text-slate-400 hover:bg-slate-800'}`}
            title="Stumble Mode"
          >
            <Shuffle size={20} />
          </button>
          <button 
            ...
            className={`p-2 rounded-lg transition-all flex items-center space-x-2 ${activeTab === 'saved' ? 'bg-rose-600 text-white shadow-lg shadow-rose-500/20' : 'text-slate-400 hover:bg-slate-800'}`}
      ...
      {viewMode === 'grid' && activeTab !== 'saved' && (
        <div className="flex overflow-x-auto pb-4 pt-2 gap-2 scrollbar-hide">
          {['All', 'Skill', 'Nature', 'Tech', 'Art', 'Craft', 'Intellectual', 'Physical', 'Culinary', 'Social', 'Adventure', 'Niche'].map((cat) => (
            <button
              key={cat}
...
const DetailModal = ({ item, onClose, toggleSaved, isSaved }) => {
  if (!item) return null;
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        {/* Panel */}
        <div className="relative inline-block align-bottom bg-slate-900 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-slate-800">
          
          {/* Header Image Area (Abstract) */}
          <div className={`h-32 w-full bg-gradient-to-br ${getGradient(item.category)} relative`}>
            <button 
              onClick={onClose}
            ...
            <div className="absolute -bottom-8 left-6">
              <div className="h-16 w-16 rounded-xl bg-slate-800 flex items-center justify-center shadow-lg border-4 border-slate-900 text-3xl">
          ...
          <div className="px-6 pt-10 pb-6">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-2xl font-bold text-white leading-6">{item.title}</h3>
                <span className="inline-flex mt-2 items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-800 text-slate-300 border border-slate-700">
                  {item.category}
                </span>
              </div>
              <button 
                onClick={() => toggleSaved(item.id)}
                className={`p-2 rounded-full transition-all ${isSaved ? 'bg-rose-600/20 text-rose-500' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
              >
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                <span className="text-xs text-slate-500 uppercase font-bold tracking-wider">Difficulty</span>
                <div className="mt-1 flex items-center space-x-2">
                  <div className={`h-2 w-2 rounded-full ${getDifficultyColor(item.difficulty)}`}></div>
                  <span className="text-slate-200 font-medium">{item.difficulty}</span>
                </div>
              </div>
              <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                <span className="text-xs text-slate-500 uppercase font-bold tracking-wider">Cost</span>
                <div className="mt-1 text-slate-200 font-medium tracking-widest">{item.cost}</div>
              </div>
            </div>

            <div className="mt-8 flex space-x-3">
              <button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 px-4 rounded-xl font-medium transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center space-x-2">
                <span>Google This</span>
                <ArrowRight size={16} />
              </button>
               <button 
                onClick={() => window.open(`https://www.youtube.com/results?search_query=how+to+start+${item.title.replace(" ", "+")}`, '_blank')}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-3 px-4 rounded-xl font-medium transition-all border border-slate-700 flex items-center justify-center space-x-2"
              >
...
const Card = ({ item, onClick, isSaved, toggleSaved }) => (
  <div 
    onClick={onClick}
    className="group relative bg-slate-800 rounded-2xl p-6 hover:bg-slate-750 border border-slate-700 hover:border-indigo-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-500/10 cursor-pointer flex flex-col h-full overflow-hidden"
  >
    {/* Abstract Background Decoration */}
    <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${getGradient(item.category)} opacity-10 rounded-bl-full -mr-10 -mt-10 transition-opacity group-hover:opacity-20`}></div>

    <div className="flex justify-between items-start mb-4 relative z-10">
      <div className="h-12 w-12 rounded-xl bg-slate-900 border border-slate-700 flex items-center justify-center text-2xl shadow-sm group-hover:scale-110 transition-transform duration-300">
        {getEmoji(item.category)}
      </div>
      <button 
        onClick={(e) => { e.stopPropagation(); toggleSaved(item.id); }}
        className={`p-2 rounded-full transition-all opacity-0 group-hover:opacity-100 ${isSaved ? 'opacity-100 text-rose-500' : 'text-slate-500 hover:bg-slate-700 hover:text-rose-400'}`}
      >
    ...
    <div className="flex-1 relative z-10">
      <h3 className="text-lg font-bold text-white mb-2 line-clamp-1 group-hover:text-indigo-300 transition-colors">{item.title}</h3>
      <p className="text-sm text-slate-400 line-clamp-3 leading-relaxed">{item.desc}</p>
    </div>

    <div className="mt-4 pt-4 border-t border-slate-700/50 flex items-center justify-between text-xs font-medium text-slate-500 relative z-10">
      <span className="flex items-center space-x-1.5">
        <span className={`w-1.5 h-1.5 rounded-full ${getDifficultyColor(item.difficulty)}`}></span>
...
const getEmoji = (cat) => {
  const map = {
    'Skill': '⚡', 'Nature': '🌿', 'Tech': '🤖', 'Art': '🎨', 
    'Craft': '🧶', 'Physical': '💪', 'Intellectual': '🧠', 'Social': '💬',
    'Adventure': '🗺️', 'Culinary': '🍳', 'Niche': '🦄', 'Gaming': '🎮',
    'Music': '🎵', 'Sport': '⚽', 'Creative': '✨', 'Science': '🔬'
  };
  return map[cat] || '📦';
};
...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState('grid'); // 'grid' | 'stumble'
  const [savedIds, setSavedIds] = useState([]);
  const [selectedItem, setSelectedItem] = useState(null);
  
  // Stumble Mode State
  const [stumbleIndex, setStumbleIndex] = useState(0);

  // Load saved from local storage (mock)
  useEffect(() => {
  ...
  const toggleSaved = (id) => {
    ...
    localStorage.setItem('rabbitHoleSaved', JSON.stringify(newSaved));
  };
  ...
  const filteredItems = useMemo(() => {
    ...
    // 1. Filter by Tab
    if (activeTab === 'saved') {
      items = items.filter(i => savedIds.includes(i.id));
    ...
    return items;
  }, [searchTerm, activeTab, savedIds]);

  // Handle Stumble
  const currentStumbleItem = useMemo(() => {
    // For stumble, we might want to stumble through ALL items, or just filtered ones
    // Let's stumble through filtered ones so the category filters still apply
    if (filteredItems.length === 0) return null;
    return filteredItems[stumbleIndex % filteredItems.length];
  }, [filteredItems, stumbleIndex]);

  const handleNextStumble = () => {
    // Randomize next index to feel like proper shuffling
    const nextIndex = Math.floor(Math.random() * filteredItems.length);
    setStumbleIndex(nextIndex);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500/30">
      <Header 
        ...
        totalItems={RAW_INTERESTS.length}
      />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {filteredItems.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-xl font-medium text-white">No interests found</h3>
        ...
        {/* STUMBLE VIEW */}
        {viewMode === 'stumble' && currentStumbleItem && (
          <div className="flex flex-col items-center justify-center min-h-[70vh] animate-in slide-in-from-bottom-4 duration-500">
            <div className="w-full max-w-2xl">
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
                {/* Background glow */}
                <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${getGradient(currentStumbleItem.category)}`}></div>
                
                <div className="flex justify-between items-start mb-8">
                   <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-slate-800 text-slate-300 border border-slate-700">
                    {currentStumbleItem.category}
                  </span>
                  <button 
                    onClick={() => toggleSaved(currentStumbleItem.id)}
                    className={`p-3 rounded-full bg-slate-800 border border-slate-700 transition-all ${savedIds.includes(currentStumbleItem.id) ? 'text-rose-500' : 'text-slate-400 hover:text-white'}`}
                  >
                    <Heart size={24} className={savedIds.includes(currentStumbleItem.id) ? 'fill-current' : ''} />
                  </button>
                </div>

                <div className="text-center mb-10">
                  <div className="inline-flex items-center justify-center h-24 w-24 rounded-2xl bg-slate-800 text-6xl mb-6 shadow-xl border border-slate-700">
                    {getEmoji(currentStumbleItem.category)}
                  </div>
                  <h2 className="text-4xl md:text-5xl font-black text-white mb-6 tracking-tight">{currentStumbleItem.title}</h2>
                  <p className="text-xl text-slate-400 leading-relaxed max-w-lg mx-auto">
                    {currentStumbleItem.desc}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8 max-w-md mx-auto">
                   <div className="bg-slate-800/50 p-4 rounded-xl text-center">
                    <span className="text-xs text-slate-500 uppercase font-bold tracking-wider">Difficulty</span>
                    <div className="mt-1 text-slate-200 font-bold">{currentStumbleItem.difficulty}</div>
                  </div>
                   <div className="bg-slate-800/50 p-4 rounded-xl text-center">
                    <span className="text-xs text-slate-500 uppercase font-bold tracking-wider">Cost</span>
                    <div className="mt-1 text-slate-200 font-bold">{currentStumbleItem.cost}</div>
                  </div>
                </div>

                <div className="flex justify-center space-x-4">
                  <button 
                    onClick={() => setSelectedItem(currentStumbleItem)}
                    className="px-8 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold transition-all border border-slate-700 flex items-center space-x-2"
                  >
                    <Maximize size={20} />
                    <span>Details</span>
                  </button>
                  <button 
                    onClick={handleNextStumble}
                    className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/25 flex items-center space-x-2 transform active:scale-95"
                  >
                    <Dice5 size={20} />
              ...
              <p className="text-center text-slate-500 mt-6 text-sm">
                Showing {stumbleIndex + 1} of {filteredItems.length} filtered items
              </p>
            </div>
          </div>
        )}
